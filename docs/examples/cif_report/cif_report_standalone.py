...
# source: analysis.py
# shows:  Aspirin (COD 7247819): CIF validation table, calculated Cu K-alpha powder pattern with an independent cross-check, and an ORTEP view with hydrogen bonds
...
def main():
    ...
    s = load(cfg)
    rep = validate(s, cfg)
    print(table_text(rep))
    write_csv(rep, os.path.join(cfg.output_dir, "hbonds.csv"))

    # ---- 2. calculated powder pattern (Cu K-alpha) + peak list
    fig, ax, pat = plot(s, cfg)
    audit_layout(fig, cfg)
    save_fig(fig, os.path.join(cfg.output_dir, "pxrd"), cfg)
    rows = peak_table(s, cfg)
    write_peaks_csv(rows, os.path.join(cfg.output_dir, "peaks.csv"))
    ...
    save(render_hbond_environment(s, cfg),
