...
# source: analysis.py
# shows:  Pectin DM by ATR-FTIR: ester/(ester+carboxylate) band-area ratio calibration on six standards (Wang 2023, Mendeley 10.17632/gkwbp3wc49.1)
...
def main():
    ...
    for name in STANDARDS:
        ...
        check_ingest(path, cfg)
        x, y = load_xy(path)
        check_trace(x, y, cfg, name=name)
    ...
    for name, (x, y) in traces.items():
        areas = {b["name"]: b["area"] for b in integrate_bands(x, y, cfg)}
    ...
    apply_style(cfg)
    fig, axes = figure(cfg, 2, 2, width_ratios=[1.0, 1.15], height_ratios=[3.2, 1.0])
    ...
    try:
        txt = methods_report(cfg, results={"slope": cal["slope"], "r2": cal["r2"], "n": cal["n"],
