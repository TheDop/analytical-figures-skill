...
def test_nested_scan_equals_from_scratch(data, method, pls_scale, capsys):
    X, y, groups = data()
    ...
    with warnings.catch_warnings():
        ...
        slow = cm.component_scan(X, y, cfg, _nested=False, **kw)
    ...
    assert "falling back" not in capsys.readouterr().out   # the self-check never fired
