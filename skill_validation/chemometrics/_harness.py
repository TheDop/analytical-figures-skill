"""Tiny zero-dep test harness for the chemometrics validation suite.

Each test_*.py exposes `collect() -> list[Result]`. A Result records the tolerance
TIER it was judged at and the PROVENANCE of the reference number, so the run doubles
as reproducibility evidence (see SPEC.md). No pytest dependency.
"""
from __future__ import annotations
import numpy as np

# default tolerances per tier (a test may override rtol/atol)
_TIER_TOL = {
    "T0": dict(rtol=1e-12, atol=1e-12),   # closed-form / hand-derived
    "T1": dict(rtol=1e-8,  atol=1e-10),   # same-language reference (sklearn/scipy/chemotools)
    "T2": dict(rtol=2e-2,  atol=1e-6),    # cross-language / published literal (override per test)
...
def close(area, name, got, want, tier="T1", provenance="", rtol=None, atol=None, note=""):
    """Numeric closeness: PASS if max abs diff <= atol OR max rel diff <= rtol."""
