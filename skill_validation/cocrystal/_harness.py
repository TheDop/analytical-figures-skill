"""Tiny zero-dep test harness for the cocrystal validation suite.
...
_TIER_TOL = {
    ...
    "T1": dict(rtol=1e-8,  atol=1e-10),   # same-language reference (scipy/sklearn)
    "T2": dict(rtol=2e-2,  atol=1e-6),    # cross-language / published literal (override per test)
    "T3": dict(rtol=0.0,   atol=0.0),     # property / invariant / regime
}
